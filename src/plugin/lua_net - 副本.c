...
static int _readProtocol(lua_State *L, const char* key)
{
    const char* str;
    int protocol;
    ...
    if (0 == strcmp(str, "tcp")){
        ...
        protocol = 0;
        luaL_error(L, "protocol type error, options \"tcp\" or \"udp\"\n");
    }
    lua_pop(L, 1);
...
static int _serverSocketCreate(struct NET_Server *srv)
{
    ...
    //设置异步
    ret = ioctlsocket(fd, FIONBIO, (unsigned long *)&opt);//设置成非阻塞模式。   
    if (0 != ret){
        return ERR_FAILED;
    }
...
static const struct luaL_Reg g_netlib [] = {
	{"createServer", _serverCreate},
	{"createClient", _connCreate},
	{NULL, NULL}
};
